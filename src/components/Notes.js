...
const Notes = (props) => {
    ...
    useEffect(() => {
        if (localStorage.getItem('token')) {
            getNotes()              // all notes have been fetched
        }      
        else{
            history('/login')
        }
    ...
    const updateNote = (currentNote) => {      // gives the modal
        ref.current.click();
    ...
    const onChange = (e) => {
        setNote({...note, [e.target.name]: e.target.value})    // name of <input> == value (whatever entered)
    }
